...
description: 康斯特 ConST820 打压设备通信协议参考，涵盖简化SCPI命令集、单位代码映射、响应解析和开发扩展指南。
---
...
function parsePressure(response):
    ...
    return { value, unit: currentDeviceUnit }  // 使用之前查询的单位
```
...
示例：假设固件新增支持 bar（代码 5）：
```
代码映射添加: 5 → bar
响应解析添加: "5" → BAR
设置命令参数: UNIT:PRESsure 5
```
